...
namespace QuanLyPhongKham.Pages.TestResultPage
{
    public class CreateModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            try
            {
                // Get new test result template from API
                var newTestResultResponse = await _httpClient.GetAsync($"{_apiBaseUrl}/api/TestResult/new");
                if (newTestResultResponse.IsSuccessStatusCode)
                {
                    var content = await newTestResultResponse.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        ModelState.AddModelError(string.Empty, errorObj?.message?.ToString() ?? "Failed to create test result.");
                    }
                }
            }
            catch (Exception ex)
            {
                ModelState.AddModelError(string.Empty, $"Error creating test result: {ex.Message}");
            }
